...
class Service:
    ...
    def create_embedding(self, file_content, course_id):
        ...
        try:
            ...
            # Start save_course_summary in a new thread
            threading.Thread(target=self.summarizer.save_course_summary, args=(course_id, extracted_text, self.course_summary)).start()
            
            # After saving to DB, create embeddings in FAISS
            threading.Thread(target=self.store_vector, args=(course_id, str(file_id), extracted_text)).start()
